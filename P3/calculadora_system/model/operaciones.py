...
from P3.proyectos_tkinter.normal.calc import operaciones
...
class Operaciones:
  ...
  def consultar():
        try:
          cursor.execute("select * from operaciones where id=%s")
  ...
  def actualizar(numero1,numero2,signo,resultado,id):
       try:
         cursor.execute(
            "update operaciones set numero1=%s, numero2=%s,signo=%s,resultado=%s,fecha=Now() where id=%s",
